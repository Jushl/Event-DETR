...
from config.Event_DETR.EDT_B import STEPS, ALPHA, VTH, TAU, Pt

'''
class SpikeAct(torch.autograd.Function):  # -1，0，1
    alpha = ALPHA
    @staticmethod
    def forward(ctx, input):
        ctx.save_for_backward(input)
        pos = ((input - VTH) > 0).float() * 1
        neg = ((input + Pt * VTH) < 0).float() * -1
        output = pos + neg
        return output.float()
    ...
    def backward(ctx, grad_output):
        ...
        pos_inp = ((input - VTH) > 0).float() * input
        neg_inp = ((input + Pt * VTH) < 0).float() * input
        out = pos_inp + neg_inp
        Kr_pos = (abs(out) < SpikeAct.alpha).float() + (out > 0).float()
        Kr_neg = (abs(out) < SpikeAct.alpha).float() + (out < 0).float()
        Kr = Kr_pos + Kr_neg * -1

        Kr = Kr.float() / (2 * SpikeAct.alpha)
        return grad_input * Kr
...
class SpikeAct(torch.autograd.Function):  # 0，1
    alpha = ALPHA
    @staticmethod
    def forward(ctx, input):
        ctx.save_for_backward(input)
...
class tdBatchNorm(nn.BatchNorm2d):
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            n = input.numel() / input.size(1)  # 28 * 28 * 2 * 4
            with torch.no_grad():
