...
from models.EDT.backbone.position_encoding import build_position_encoding
from models.EDT.backbone.Convnext.convnext import build_convnext
from models.EDT.backbone.Swin.swin_transformer import build_swin_transformer
from models.EDT.backbone.ResNet.resnet import resnet50
from models.EDT.backbone.SNN.snn import resnet50_snn
...
class BackboneBase(nn.Module):
    def __init__(self, backbone: nn.Module, train_backbone: bool, num_channels, return_interm_indices: list, name=None):
        super().__init__()
    ...
    def forward(self, tensor_list: NestedTensor):
        if self.name == 'resnet50-snn':
            ...
            tensor_list.tensors = tensor_list.tensors.view(B_size, -1, H, W, N_steps)
...
class Backbone(BackboneBase):
    def __init__(self, name: str, train_backbone: bool, return_interm_indices: list, steps: int):
        if name == 'resnet50':
            ...
            backbone = resnet50_snn(return_interm_indices=return_interm_indices, pretrained=train_backbone, steps=steps)
        else:
        ...
        super().__init__(backbone=backbone, train_backbone=train_backbone, num_channels=num_channels,
                         return_interm_indices=return_interm_indices, name=name)
...
def build_backbone(args):
    ...
    if args.backbone in ['resnet50', 'resnet50-snn']:
        backbone = Backbone(name=args.backbone, train_backbone=train_backbone, return_interm_indices=return_interm_indices, steps=args.STEPS)
