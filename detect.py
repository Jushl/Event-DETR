...
from models.EDT.dino import build_dino
...
def detect(args, npy, size=None):
    ...
    for i in range(len(tgt['box_label'])):
        ...
        conf = tgt['conf'][i]
        if int(label_num) == 0: label, color = 'car', (255, 0, 0)
        elif int(label_num) == 1: label, color = 'pedestrian', (0, 0, 255)
        lab_and_conf = f'{label:} {conf:.2f}'
        plot_one_box(xyxy, img, label=lab_and_conf, color=color, line_thickness=1)

    # cv2.imshow('detect', img)
    cv2.imwrite(os.path.join(f'./weights/detect/{args.dataset_file}/', npy.split('/' or '\\')[-1].replace('.npy', '.png')), img)
...
if __name__ == '__main__':
    ...
    npy = 'weights/events/gen1/17-10-19_09-55-10_610500000_670500000_12749999.npy'
    detect(args, npy, size=455)
