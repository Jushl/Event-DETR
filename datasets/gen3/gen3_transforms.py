...
def process_data(data, n_segments):
    ...
    outputs = torch.zeros(n_segments, 480, 640)
    max_limit = (data[:, 0].max(), data[:, 1].max())
    assert max_limit <= (480, 640), '超出限制了!!!'
    for i in range(n_segments):
        ...
        for j, row in enumerate(seg_data):
            ...
            if coords != [0, 0]:
                outputs[i, max(coords[0] - 1, 0), max(coords[1] - 1, 0)] += row[3].astype(np.float32)
    return outputs
...
def stream_to_tensor(data=None, num_channel=None):
    ...
    time_data = time_entropy + time_sequence
    polarity *= time_data
    data[:, 3] = polarity
...
def to_tensor(pic, num_channel=1, mapping=True) -> Tensor:
    ...
    normalized_event = event.view(num_channel, 480, 640)
    ...
    # end_time = time.time()
    # print('CSE Processing Time：', end_time - start_time)
...
def to_tensor(pic, num_channel=1, mapping=True) -> Tensor:
    ...
    default_float_dtype = torch.get_default_dtype()
    event = stream_to_tensor(pic, num_channel).contiguous()
    event = event.view(num_channel, -1)
    evs = []
    for num in range(num_channel):
        ev = event[num]
        pos_norm_ev = ev[ev > 0]
        pos_mean = pos_norm_ev.mean()
        neg_norm_ev = ev[ev < 0]
        neg_mean = neg_norm_ev.mean()
        ev = torch.clamp(ev, min=neg_mean * 1.5, max=pos_mean * 1.5)

        pos_norm_ev = ev[ev > 0]
        pos_mean = pos_norm_ev.mean()
        pos_var = pos_norm_ev.var()
        neg_norm_ev = ev[ev < 0]
        neg_mean = neg_norm_ev.mean()
        neg_var = neg_norm_ev.var()
        ev = torch.clamp(ev, min=neg_mean - 1 * neg_var, max=pos_mean + 1 * pos_var)
        max, min = torch.max(ev, dim=0)[0], torch.min(ev, dim=0)[0]
        ev[ev > 0] /= max
        ev[ev < 0] /= abs(min)
        evs.append(ev)


    event = torch.stack(evs, dim=0)
...
if __name__ == '__main__':
    ...
    trans1_drop = Drop()
    trans2_totensor = ToTensor(time_step=2)
    trans3_flip = RandomHorizontalFlip()
    trans4_resize = RandomResize([224, 240, 256, 288, 320], 433)
    trans4_resize2 = RandomResize([200, 250, 300], 433)
    trans5_crop = RandomSizeCrop(*[200, 300])
    ...
    def gen_random_event(event, root_path, target):
        if event.split('.')[-1] == 'npy':
            target['boxes'][:, 2:] += target['boxes'][:, :2]
            event_data, target = trans1_drop(event, target)
            event_data, target = trans2_totensor(event_data, target)
            event_data, target = trans3_flip(event_data, target)
            event_data, target = trans4_resize2(event_data, target)
            # event_data, target = trans5_crop(event_data, target)
            # event_data, target = trans4_resize(event_data, target)

            if event[0] != 1:
                event_array = event_data[0].numpy()
                # event_array = torch.sum(event_data, dim=0).numpy()
            else:
                event_array = event_data.numpy().squeeze(0)
            return event_array, target['boxes']
        else:
            events_path = os.path.join(root_path, '70000train', 'events')
            npy_list = os.listdir(events_path)
            event_name = random.choice(npy_list)
            event = os.path.join(events_path, event_name)
            event_data, _ = trans2_totensor(event, None)
            event_array = event_data.numpy().squeeze(0)
            print(event)
            return event_array

    def gen_event_bbox(root_path):
        annotations_file = os.path.join(root_path, 'annotations_npy', '70000_train_annotations.json')
        ...
        for ann in annotations:
            if ann['image_id'] == event_id:
                bboxes.append(ann['bbox'])
        return event_name, bboxes

    set_random(42)
    root_path = '/home/WangXiaoZhen/PublicData/GEN1/Gen1_30000'
    event_name, bboxes = gen_event_bbox(root_path)
    event_array, target_boxes = gen_random_event(os.path.join(root_path, '70000train', 'events', event_name), root_path, {'boxes': torch.Tensor(bboxes)})
    event_visual = Image.fromarray(event_array.astype('uint8')).convert('L')

    if event_name.split('.')[-1] == 'npy':
        draw = ImageDraw.Draw(event_visual)
